...
lib.deleteFolderRecursive("./csResult")
lib.createDir("./csResult")
//Getting the STM files

copyDir.sync(grntFolder, "./csResult/");
copyDir.sync(kdbFolder, "./csResult/");

for(key in map){
  if(fs.existsSync("./csResult/" + key)){
    if(fs.existsSync("./csResult/" + map[key])){
      //console.log("Exists : " + key + " map[key] : " + map[key])
      copyDir.sync("./csResult/" + key, "./csResult/" + map[key])
      lib.deleteFolderRecursive("./csResult/" + key)
    } else {
